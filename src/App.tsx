...
function App() {
  ...
  const {get, patch, post, loading, remove} = useFetch();
  ...
  const clearTasks = () => {
    const tasksUncompleted = tasks.filter(item => item.isChecked === false);
    ...
    <main className="w-screen min-h-screen max-h-auto bg-[url('./assets/bg-desktop-dark.jpg')] bg-no-repeat pb-4 bg-[#171823] flex justify-center" >
      <div className="w-[540px] h-auto mt-[50px]" >
        ...
        {loading === false && notFound === false && tasks?.map((item, index) => (<article onMouseEnter={()=>setTaskIndex(index)} className={`w-full ${index === 0 && 'rounded-tl-[5px] rounded-tr-[5px]'} border-b-[#393A4B] border-solid border-b h-[64px] bg-[#25273D] flex items-center justify-between`} >
          <div className="flex items-center" >
            <span data-testid="checkItem" onClick={()=> checkTask(item?.id)} className={`w-[24px] h-[24px] border-solid border flex items-center justify-center border-[#393A4B] ml-[24px] rounded-full ${item?.isChecked ? 'bg-gradient-to-r from-[#55DDFF] to-[#C058F3]' : 'bg-[#25273D]'}`} >
              {item?.isChecked &&<FaCheck className="text-white text-xs" />}
            </span>
            <p className={`text-sm  ml-[24px] ${item?.isChecked ? 'line-through text-[#4D5067]' : 'text-[#C8CBE7]'}`} >{item?.text}</p>
          </div>
          {taskIndex === index && <IoCloseSharp onClick={()=> removeTask(item?.id)} className="mr-[24px] cursor-pointer text-2xl text-[#5B5E7E] hover:text-white" />}
        </article>))}
        ...
        <article className="w-full h-[55px] rounded-bl-[5px] rounded-br-[5px] bg-[#25273D] px-[24px] flex items-center justify-between" >
          ...
          <div className="flex justify-between w-36" >
            <p onClick={()=>{setClickAll(true), setClickActive(false), setClickCompleted(false)}} className={`text-xs cursor-pointer hover:text-white ${clickAll ? 'text-[#3A7CFD]' : 'text-[#5B5E7E]'}`} >All</p>
            <p onClick={()=>{setClickAll(false), setClickActive(true), setClickCompleted(false)}} className={`text-xs cursor-pointer hover:text-white ${clickActive ? 'text-[#3A7CFD]' : 'text-[#5B5E7E]'}`} >Active</p>
            <p onClick={()=>{setClickAll(false), setClickActive(false), setClickCompleted(true)}} className={`text-xs cursor-pointer hover:text-white ${clickCompleted ? 'text-[#3A7CFD]' : 'text-[#5B5E7E]'}`} >Completed</p>
          </div>

          <p onClick={()=> clearTasks()} className="text-xs text-[#5B5E7E] cursor-pointer hover:text-white" >Clear Completed</p>
        </article>
